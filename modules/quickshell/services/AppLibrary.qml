...
Item {
  ...
  function launch(desktopId, name) {
    ...
    // Start gtk-launch inside UWSM when it exists. Plain Sway/Labwc/KDE
    // sessions do not necessarily provide uwsm-app, so the same launcher
    // falls back to gtk-launch without changing the desktop-entry resolver.
    // Keep the .desktop suffix or ids like org.telegram.desktop won't resolve.
    Util.execGraphical(["gtk-launch", id + ".desktop"])
  }

  function remove(desktopId, name) {
    var id = String(desktopId || "")
    if (!id) return
    Util.execDetached(Util.shellQuote(root.shellRoot + "/services/omarchy-remove-launcher-entry") + " " + Util.shellQuote(id) + " " + Util.shellQuote(String(name || id)))
  }
  ...
  function hiddenEntryScanCommand() {
    ...
    var script = root.shellRoot + "/services/hidden-entries.sh"
  ...
  FileView {
    path: root.shellRoot + "/default/omarchy/launcher.hides"
    watchChanges: true
    printErrors: false
  ...
  Timer {
    ...
    onTriggered: {
      ...
      // Use the desktop notification protocol so launch feedback works with
      // Labwc, Sway, Hyprland, and other Wayland compositors alike.
      Quickshell.execDetached(["notify-send", "-u", "low", "-t", "15000", "󱓞", root.launchOsdMessage])
    }
  }
