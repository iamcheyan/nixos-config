...
Panel {
  ...
  Process {
    id: batteryProc
    command: [Qt.resolvedUrl("../../../services/quickshell-battery-status").toString().replace("file://", "")]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateKeyValue(text, "battery") }
  }

  Process {
    id: profilesProc
    command: [Qt.resolvedUrl("../../../services/quickshell-power-profiles").toString().replace("file://", "")]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateProfiles(text) }
  }

  Process {
    id: systemProc
    command: [Qt.resolvedUrl("../../../services/quickshell-system-stats").toString().replace("file://", "")]
  ...
  Process {
    id: hibernateCheck
    command: ["bash", "-c", "grep -qw disk /sys/power/state && swapon --noheadings 2>/dev/null | grep -v zram | grep -qw partition && echo YES || echo NO"]
  ...
  BarIconButton {
    ...
    text: root.batteryPresent && root.showPercentage && !vertical
      ...
      : root.batteryPresent ? root.batteryIcon() : "⏻"
      ...
      Column {
        ...
        Item {
          ...
          Column {
            ...
            Text {
              id: heroStatus
              textFormat: Text.PlainText
              text: root.batteryPresent ? root.heroStatusText.toUpperCase() : (root.activeProfile || "desktop power").toUpperCase()
          ...
          Text {
            id: heroPercent
            textFormat: Text.PlainText
            text: root.batteryPresent ? (root.batteryInfo.percentage || "—") : (root.activeProfile || "—")
            color: root.bar.foreground
            font.family: root.bar.fontFamily
        ...
        Column {
          ...
          Grid {
            ...
            Button { width: (parent.width - parent.columnSpacing) / 2; iconText: "󰌾"; text: "Lock"; foreground: root.bar.foreground; onClicked: root.runSessionAction("lock") }
