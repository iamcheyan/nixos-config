...
Item {
  ...
  function launchScreensaver() {
    if (!root.hyprlandSession) {
      root.logEvent("screensaver-skip", "no compositor-neutral screensaver configured")
      return
    }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", root.hyprlandSession ? "omarchy-system-lock" : "swaylock -f -c 000000")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle && root.hyprlandSession)
