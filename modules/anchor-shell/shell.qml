...
ShellRoot {
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("integrationPath" in target) target.integrationPath = shell.integrationPath
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("integrationPath" in item) item.integrationPath = shell.integrationPath
  ...
  function reloadPlugins() {
    var lockId = shell.pluginRegistry.resolveEnabledId("omarchy.lock")
    var lockService = shell.serviceFor(lockId)
    if (lockService && lockService.locked) {
      console.warn("Deferring plugin reload while session lock is active")
      return
    }
  ...
  IpcHandler {
    ...
    // Compositor-neutral active-window information for external helpers such
    // as the Voxtype paste hook. ToplevelManager works on Labwc as well as
    // Hyprland/Sway, unlike compositor-specific IPC commands.
    function activeAppId(): string {
      var active = ToplevelManager.activeToplevel
      // Some wlroots compositors expose the activated foreign toplevel before
      // Quickshell updates activeToplevel.  Prefer that activated entry as a
      // fallback so external hooks do not mistake a terminal for a browser.
      if (!active) {
      ...
      if (!active) return ""
      return String(active.appId || active.initialAppId || "")
    }
