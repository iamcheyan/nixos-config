...
2. See how an existing theme is done via `$OMARCHY_PATH/themes/catppuccin`.
...
Stock themes live under `$OMARCHY_PATH/themes/` — a read-only store path, so
editing one is not possible. Two safe options:
...
cp --no-preserve=mode "$OMARCHY_PATH"/themes/catppuccin/colors.toml ~/.config/omarchy/themes/catppuccin/
...
```bash
cp -r --no-preserve=mode "$OMARCHY_PATH"/themes/catppuccin ~/.config/omarchy/themes/catppuccin-custom
