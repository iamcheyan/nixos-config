# `devenv-init <preset>` -- a fresh folder becomes a working, pinned
...
let
  presets = import ./devenv-presets.nix;
  ...
  # variable could.
  presetDir = runCommand "local-devenv-presets" { } (
    ''
      mkdir -p $out
      cp ${
        writeText "index.tsv" (
...
writeShellApplication {
  ...
  text = ''
    ...
    list() {
      ...
      echo
      echo "  devenv-init <preset>"
    }

    case "''${1:-}" in
      ""|-h|--help|--list|help) list; exit 0 ;;
    esac

    preset="$1"
    if [ ! -f "$presets/$preset.nix" ]; then
      echo "devenv-init: no preset '$preset'." >&2
      list >&2
      exit 1
    fi

    if ! command -v devenv >/dev/null 2>&1; then
      echo "devenv-init: devenv is not installed, and this command is a wrapper" >&2
      echo "around it -- there is nothing useful to do without it." >&2
      echo >&2
      echo "Enable modules/devenv.nix in ~/nixos-config and rebuild this host." >&2
      exit 1
    fi
    ...
    if [ -e devenv.nix ]; then
      echo "devenv-init: this directory already has a devenv.nix." >&2
